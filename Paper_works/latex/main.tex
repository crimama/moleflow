...
\title{\method{}: Structural Decomposition of \\Normalizing Flows for \\Continual Anomaly Detection}
...
% Integral Components
To compensate for frozen base rigidity, we introduce three integral components validated through interaction effect analysis: General Distribution Adaptation for distribution alignment, Tail-Aware Loss for decision boundary focus, and Affine Coupling Layer for nonlinear manifold correction.
...
\begin{itemize}
    ...
    \item Introduces three \textbf{integral components}---General Distribution Adaptation (GDA), Tail-Aware Loss (TAL), and Affine Coupling Layer (ACL)---that specifically compensate for frozen base rigidity, validated as providing significantly \emph{amplified benefits under the frozen constraint} via interaction effect analysis.
...
$\gamma_t, \beta_t$ & General Distribution Adaptation parameters & $\mathbb{R}^D$ \\
...
\begin{equation}
\mathbf{x}_{\text{img}} \xrightarrow{\text{Backbone}} \mathbf{F}^{(0)} \xrightarrow{\text{PE+GDA}} \mathbf{F}^{(1)} \xrightarrow{\text{SCM}} \mathbf{F}^{(2)} \xrightarrow{\text{MoLE-NF}} \mathbf{z} \xrightarrow{\text{ACL}} (\mathbf{z}', \log|\det \mathbf{J}|),
\end{equation}
where PE is positional encoding, GDA is General Distribution Adaptation, SCM is Spatial Context Mixer, MoLE-NF is our LoRA-equipped normalizing flow, and ACL is Affine Coupling Layer.

\begin{figure}[t]
\centering
% [Architecture Diagram Description]
% Left panel: Full pipeline showing frozen WideResNet-50-2 backbone extracting multi-scale features,
% followed by Positional Encoding and General Distribution Adaptation (task-specific), then Spatial Context Mixer.
...
% Bottom: ACL blocks (2 coupling layers) for task-specific nonlinear correction.
...
PE+GDA preprocessing, Spatial Context Mixer, to MoLE-NF and ACL blocks.\\[0.5em]
...
\caption{Overview of \method{} architecture. Coupling layer subnets are decomposed into frozen shared bases and task-specific LoRA adapters, enabling complete parameter isolation. Integral components (GDA, ACL) compensate for frozen base rigidity. The router selects the appropriate task expert based on Mahalanobis distance to learned prototypes.}
\label{fig:architecture}
\end{figure}

\paragraph{Training Strategy.}
\begin{itemize}
    \item \textbf{Task 0:} Train base NF weights $\Theta_{\text{base}}$ and task-specific components (LoRA$_0$, GDA$_0$, ACL$_0$) jointly.
    \item \textbf{Task $t \geq 1$:} Freeze $\Theta_{\text{base}}$; train only (LoRA$_t$, GDA$_t$, ACL$_t$).
\end{itemize}
...
% ----------------------------------------------------------------------------
% Why Linear LoRA + ACL Instead of Nonlinear LoRA
% ----------------------------------------------------------------------------
\paragraph{Why Linear LoRA with ACL Instead of Nonlinear LoRA.}
An alternative to our design would be using nonlinear LoRA (e.g., with intermediate activations).
We chose linear LoRA combined with ACL for three reasons:
(1) \textbf{Simplicity}: Linear LoRA has well-understood optimization dynamics and initialization strategies;
(2) \textbf{Modularity}: Separating linear task adaptation (LoRA) from nonlinear correction (ACL) provides cleaner ablation and interpretability;
(3) \textbf{Empirical effectiveness}: Our ablations show ACL provides +5.2\%p P-AP under frozen base, suggesting the nonlinear capacity is better placed after base transformation.
...
Per-task parameters: LoRA ($2 \times r \times D$ per layer $\times$ 24 layers) + TaskBias + GDA $\approx$ 1.93M (\textbf{21.8\%} of NF base), or 3.71M (\textbf{41.7\%}) including ACL.
...
\subsubsection{General Distribution Adaptation (GDA): Distribution Alignment}
\label{sec:method:wa}
...
GDA provides +7.3\%p P-AP under frozen base vs. +1.2\%p unfrozen---a 6$\times$ amplification.
...
\subsubsection{Affine Coupling Layer (ACL): Nonlinear Manifold Correction}
...
ACL uses 2 affine coupling blocks with fully task-specific parameters (no sharing), enabling nonlinear distribution normalization.

\paragraph{Placement Logic.}
ACL is placed \emph{after} the base NF (not within coupling layers) for two reasons:
(1) Base NF applies universal transformation first; ACL performs task-specific adjustment on the already-transformed latent space;
(2) Placing ACL after allows complete task-specific parameters without interference with shared base weights.

\paragraph{Integral Status.}
Interaction effect: $F(1,16) = 5.12$, $p = 0.034$, partial $\eta^2 = 0.24$.
ACL provides +5.2\%p P-AP under frozen base vs. +2.1\%p unfrozen.
...
% Panel (a) GDA: Lines diverge significantly under Frozen condition (gap ~7.3%p) vs. minimal
...
% Panel (c) ACL: Gap of ~5.2%p under Frozen vs. ~2.1%p Unfrozen.
...
(a) GDA: +7.3\%p under frozen vs. +1.2\%p unfrozen (6$\times$ amplification)\\
(b) TAL: +7.6\%p under frozen vs. +3.2\%p unfrozen (2.4$\times$ amplification)\\
(c) ACL: +5.2\%p under frozen vs. +2.1\%p unfrozen (2.5$\times$ amplification)\\[0.5em]
...
\end{equation}
then activate corresponding LoRA$_{t^*}$, GDA$_{t^*}$, ACL$_{t^*}$.
...
w/o GDA & 97.90 & -0.02 & 48.84 & \textbf{-7.34} & 0.0 \\
w/o ACL & 92.74 & \textbf{-5.18} & 50.06 & -6.12 & 0.0 \\
...
(2) \textbf{GDA} similarly contributes +7.34\%p P-AP by normalizing distribution shifts.
(3) \textbf{ACL} is essential for I-AUC (+5.18\%p), providing training stability through nonlinear manifold correction.
...
For each component $C \in \{\text{GDA}, \text{TAL}, \text{ACL}\}$:
\begin{itemize}
...
\midrule
GDA & +7.3\%p & +1.2\%p & $F(1,16)=8.47$ & 0.008* & 6.1$\times$ \\
TAL & +7.6\%p & +3.2\%p & $F(1,16)=6.23$ & 0.021* & 2.4$\times$ \\
ACL & +5.2\%p & +2.1\%p & $F(1,16)=5.12$ & 0.034* & 2.5$\times$ \\
\midrule
...
All three integral components (GDA, TAL, ACL) show statistically significant interactions ($p < 0.05$), with contributions amplified 2-6$\times$ under frozen base.
...
The 22-42\% per-task parameter overhead (depending on ACL inclusion) is higher than prompt-based methods but provides complete isolation guaranteeing zero forgetting.
...
The three integral components---General Distribution Adaptation, Tail-Aware Loss, and Affine Coupling Layer---were systematically validated as providing amplified benefits under frozen base constraints through interaction effect analysis.
...
GDA $\gamma$ range & [0.5, 2.0] & Medium \\
GDA $\beta$ range & [-2.0, 2.0] & Medium \\
\bottomrule
\end{tabular}
\end{table}
...
% ----------------------------------------------------------------------------
% D. GDA Constraint Bounds Ablation
% ----------------------------------------------------------------------------
\section{GDA Constraint Bounds Ablation}
\label{sec:supp:wa_bounds}

\begin{table}[h]
\centering
\caption{General Distribution Adaptation constraint range ablation.}
\small
\begin{tabular}{ccccc}
\toprule
...
\begin{tabular}{lcccc}
\toprule
\textbf{Rank} & \textbf{I-AUC} & \textbf{P-AP} & \textbf{LoRA Params} & \textbf{Per-Task (excl. ACL)} \\
\midrule
...
\subsection{ACL Blocks}
\label{sec:supp:dia}

\begin{table}[h]
\centering
\caption{Number of ACL blocks ablation.}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{ACL Blocks} & \textbf{Total Blocks} & \textbf{I-AUC} & \textbf{P-AP} & \textbf{Stability} \\
\midrule
...
Tasks with features $>5\sigma$ from Task 0 distribution hit GDA constraint bounds, degrading P-AP by 3-5\%p.
