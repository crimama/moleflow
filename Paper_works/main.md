...
**결론:**
파일럿 실험은 **NF의 임의 함수 속성만이 지속적 이상 탐지를 위한 파라미터 분해를 고유하게 가능하게 함**을 입증한다. 다른 아키텍처는 다음과 같은 이유로 실패한다:
1. **AE/VAE:** 인코더-디코더 coupling → 잠재 일관성 붕괴 (망각 59-81%)
2. **T-S:** Teacher-Student 정렬 → 분포 정렬 불가능 (망각 71.6%)
