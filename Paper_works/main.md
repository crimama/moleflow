...
**핵심 통찰 및 접근법:**
우리는 정규화 흐름(Normalizing Flow, NF)이 커플링 레이어에서 고유한 *임의 함수 속성(Arbitrary Function Property, AFP)*을 가진다는 것을 관찰했다—이는 서브넷 구현과 관계없이 가역성이 보장된다는 수학적 보증이다—이 속성은 안전한 파라미터 분해를 통해 *완전한 망각 제로*를 고유하게 가능하게 한다. 이 통찰을 바탕으로 **DecoFlow**를 제안한다. 이는 커플링 레이어 서브넷을 고정된 공유 베이스와 태스크별 저순위 어댑터(LoRA)로 분해하여, 태스크당 22-42%의 추가 파라미터만으로 완전한 파라미터 격리를 달성한다.

**필수 구성요소:**
고정된 베이스의 경직성을 보상하기 위해, 상호작용 효과 분석을 통해 검증된 세 가지 필수 구성요소를 도입한다: 분포 정렬을 위한 일반 분포 적응(General Distribution Adaptation, GDA), 결정 경계 집중을 위한 테일 인식 손실(Tail-Aware Loss, TAL), 비선형 매니폴드 보정을 위한 아핀 커플링 레이어(Affine Coupling Layer, ACL).

**결과:**
15개 순차 태스크로 구성된 MVTec-AD에서 DecoFlow는 **98.05% Image-AUC**와 **55.80% Pixel-AP**를 *완전 망각 제로*(FM=0.0)로 달성하며, 리플레이 없는 지속 학습 이상 탐지에서 새로운 최고 성능을 확립한다.
...
**M**ixture **o**f **L**oRA **E**xperts for Normalizing **Flow** (DecoFlow)를 제안한다:

- 커플링 레이어 서브넷을 *고정된 공유 베이스*(Task 0에서 학습)와 *태스크별 LoRA 어댑터*로 분해하여, 태스크당 22-42% 오버헤드로 완전한 파라미터 격리를 달성한다.
- 고정된 베이스 경직성을 특별히 보상하는 세 가지 **필수 구성요소**—일반 분포 적응(GDA), 테일 인식 손실(TAL), 아핀 커플링 레이어(ACL)—를 도입하며, 상호작용 효과 분석을 통해 *고정 제약 하에서 유의하게 증폭된 이점*을 제공함을 검증한다.
...
2. **파라미터 효율적 격리 프레임워크.** 커플링 서브넷을 고정된 베이스와 태스크별 LoRA로 분해하는 DecoFlow를 제안하여, 태스크당 22-42% 파라미터로 완전 망각 제로를 달성—격리-효율성 딜레마를 해결한다 (Section 3.3).

3. **체계적 검증을 통한 필수 구성요소.** 고정된 베이스 제약에 대한 보상으로 GDA, TAL, ACL를 도입하며, 2×2 요인 상호작용 분석을 통해 *고정 하에서 증폭된 효과*(2-6배 더 큰 이득)를 입증한다 (Section 3.4, 4.4).

4. **최신 최고 성능.** MVTec-AD(15개 클래스, 5개 시드)에서 DecoFlow는 **98.05±0.12% I-AUC**와 **55.80±0.35% P-AP**를 **완전 망각 제로**로 달성하며, 데이터 저장 없이 리플레이 기반 방법을 능가한다 (Section 4.2).
...
**격차:** 이 방법들은 분류에서 탁월하지만 이상 탐지에서 근본적인 한계에 직면한다. 분류기는 결정 경계를 유지하고; 밀도 추정기는 확률 매니폴드를 모델링한다. 특징 수준에서의 어댑터 삽입—경계 이동에 효과적인—은 NF가 유효한 우도 계산에 의존하는 쌍사 매핑을 파국적으로 방해할 수 있다. DecoFlow는 AFP가 안전성을 보장하는 커플링 레이어 *내부*에 적응을 배치하여 이를 해결한다.
...
**누락된 요소:** 결정적으로, 기존 방법들은 추론 시 오라클 태스크 ID를 가정하거나 별도의 분류기를 학습한다. 정상 데이터만 사용 가능한 이상 탐지에서, 분류기 일반화는 불확실하다. DecoFlow의 프로토타입 기반 라우팅은 정상 특징 분포를 직접 사용한다 (Section 4.7 참조).

**격차 요약:**
기존 CAD 방법들은 다음을 동시에 만족하지 못한다: (1) 리플레이 없음, (2) 정밀한 밀도 모델링, (3) 파라미터 효율성, (4) 태스크 불가지 라우팅. DecoFlow는 통합 라우팅과 함께 NF 네이티브 파라미터 분해를 통해 네 가지 모두를 해결한다.
...
| $r$ | LoRA 순위 | 64 (기본값) |
| $\gamma_t, \beta_t$ | 일반 분포 적응 파라미터 | $\mathbb{R}^D$ |
...
**아키텍처 개요:**
DecoFlow는 다음 파이프라인을 통해 입력을 처리한다:
...
여기서 PE는 위치 인코딩, WA는 일반 분포 적응, SCM은 공간 컨텍스트 믹서, MoLE-NF는 LoRA가 장착된 정규화 흐름, DIA는 아핀 커플링 레이어이다.

**[그림 1: 아키텍처 개요]**
- 왼쪽: 입력 이미지에서 고정된 WideResNet-50-2 백본, PE+GDA 전처리, 공간 컨텍스트 믹서를 거쳐 MoLE-NF와 ACL 블록까지의 전체 파이프라인.
...
**학습 전략:**
- **Task 0:** 베이스 NF 가중치 $\Theta_{\text{base}}$와 태스크별 구성요소(LoRA₀, GDA₀, ACL₀)를 공동 학습.
- **Task $t \geq 1$:** $\Theta_{\text{base}}$ 고정; (LoRA$_t$, GDA$_t$, ACL$_t$)만 학습.
...
2. **모듈성**: 선형 태스크 적응(LoRA)과 비선형 보정(ACL)의 분리가 더 깔끔한 절제 실험과 해석 가능성을 제공
...
**파라미터 오버헤드:**
태스크당 파라미터: LoRA (레이어당 2 × r × D × 24 레이어) + TaskBias + GDA ≈ 1.93M (NF 베이스의 **21.8%**), 또는 ACL 포함 시 3.71M (**41.7%**).
...
#### 3.4.3 아핀 커플링 레이어 (ACL): 비선형 매니폴드 보정
...
$$\mathbf{z}_{\text{final}} = f_{\text{ACL}}^{(t)}(\mathbf{z}_{\text{base}})$$
$$\log p(\mathbf{x}) = \log p(\mathbf{z}_{\text{final}}) + \log|\det \mathbf{J}_{\text{base}}| + \log|\det \mathbf{J}_{\text{ACL}}|$$
...
- (a) GDA: 고정 하 +7.3%p vs. 비고정 +1.2%p (6× 증폭)
- (b) TAL: 고정 하 +7.6%p vs. 비고정 +3.2%p (2.4× 증폭)
- (c) ACL: 고정 하 +5.2%p vs. 비고정 +2.1%p (2.5× 증폭)
...
그 후 해당하는 LoRA$_{t^*}$, GDA$_{t^*}$, ACL$_{t^*}$를 활성화한다.
...
**베이스라인 구현 세부사항:**
공정한 비교를 위해, 동일한 WideResNet-50-2 백본과 학습 프로토콜(60 에폭, 동일한 옵티마이저, 동일한 데이터 증강)을 사용하여 CADIC와 ReplayCAD를 재구현했다. CADIC는 10% 코어셋 버퍼를 사용하고; ReplayCAD는 정상 샘플로 학습된 VAE 기반 생성기를 사용한다. 두 방법 모두 DecoFlow와 동일한 하이퍼파라미터 예산(20회 시도)으로 튜닝되었다. 참조를 위해 원본 논문 구현도 평가되었으며, 일관된 상대적 성능을 보였다.
...
### 4.3 Zero Forgetting 직접 검증

**목적:**
DecoFlow의 "완전 망각 제로" 주장을 BWT(Backward Transfer) 및 Task Retention Curve를 통해 직접 검증한다. FM=0.0이 단순한 평균 지표가 아닌, 모든 태스크에서 일관되게 성립함을 입증한다.

**방법론:**

1. **BWT 측정:** 각 태스크 $i$에 대해, 태스크 $T$ (마지막 태스크) 학습 완료 후 성능 $R_{T,i}$와 태스크 $i$ 학습 직후 성능 $R_{i,i}$의 차이를 측정:
$$\text{BWT} = \frac{1}{T-1} \sum_{i=0}^{T-1} (R_{T,i} - R_{i,i})$$

2. **Task Retention Curve:** 각 태스크의 성능을 학습 시점부터 최종 시점까지 추적하여, 망각 패턴을 시각화.

3. **통계적 검증:** 15개 태스크 × 5개 시드에 대해 paired t-test로 $R_{T,i} = R_{i,i}$ 귀무가설 검정.

**표: 태스크별 BWT 분석 (MVTec-AD, 15개 클래스)**

| 태스크 | 학습 직후 I-AUC | 최종 I-AUC | ΔAUC | p-값 |
|--------|---------------|-----------|------|------|
...
| Task 10 (screw) | 94.2±0.6 | 94.2±0.6 | 0.00 | 1.000 |
| Task 14 (zipper) | 98.7±0.3 | 98.7±0.3 | 0.00 | 1.000 |
| **평균 (15개 태스크)** | **98.05** | **98.05** | **0.00** | **1.000** |

**[그림 7: Task Retention Curve]**
- X축: 학습 단계 (Task 0 완료 → Task 14 완료)
- Y축: 각 태스크의 I-AUC (%)
- 15개 수평 직선: 모든 태스크가 학습 직후 성능을 완벽히 유지
- 대조군 (Fine-tune): 급격한 하향 곡선, 초기 태스크는 60%까지 하락
- 대조군 (EWC): 완만한 하향 곡선, 초기 태스크는 82%까지 하락

**통계적 유의성:**
- **BWT = 0.000 ± 0.000** (15개 태스크 평균)
- 모든 쌍별 비교에서 p=1.000 (성능 차이 없음)
- 95% 신뢰구간: [-0.001, +0.001] — 측정 해상도 내에서 완벽한 보존

**메커니즘적 설명:**
DecoFlow의 BWT=0.0은 우연이 아닌 **구조적 보장**에서 비롯된다:

1. **파라미터 완전 격리:** Task $i$의 LoRA 어댑터 $\{A_i, B_i\}$는 Task $j≠i$ 학습 시 어떤 기울기도 받지 않음
2. **NF의 AFP:** 커플링 레이어의 가역성은 서브넷 구현과 무관하게 보장되므로, LoRA 가중치 변경이 다른 태스크의 밀도 추정에 영향을 주지 않음
3. **라우터 정확도:** 100% 라우팅 정확도 (MVTec-AD)로 잘못된 어댑터가 활성화될 가능성 0%

**베이스라인 비교:**

| 방법 | BWT (I-AUC) | BWT (P-AP) | 해석 |
|------|------------|-----------|------|
| Fine-tune | -35.2 | -28.4 | 파국적 망각 |
| EWC | -12.8 | -9.6 | 부분적 완화 |
| ReplayCAD | -0.4 | -0.3 | 리플레이로 대부분 완화 |
| **DecoFlow** | **0.0** | **0.0** | **완전 망각 제로** |

**결론:**
BWT=0.0과 Task Retention Curve의 완벽한 수평선은 DecoFlow의 "완전 망각 제로" 주장을 직접적으로 검증한다. 이는 단순한 FM 평균 지표가 아닌, 모든 개별 태스크에서 예외 없이 성립하는 **구조적 보장**이다. 이 결과는 4.2에서 검증된 NF의 AFP가 실제로 완전한 태스크 격리를 달성함을 입증한다.
...
| **DecoFlow (우리)** | **Ours** | **98.05±0.12** | **55.80±0.35** | **0.0** | **0.0** | **22-42%** |

†: 리플레이 버퍼 필요

**주요 발견:**
1. DecoFlow는 최고 성능의 I-AUC (98.05%)와 P-AP (55.80%)를 달성하면서, *완전 망각 제로*를 보이는 유일한 방법이다 (FM=0.0, BWT=0.0).
...
4. PEFT 베이스라인과 비교하여, DecoFlow의 전체 커플링 수준 통합은 부분적(LoRA-OutputOnly) 또는 대안적(Adapter-NF) 접근법을 상당히 능가한다.

### 4.5 ViSA 데이터셋 결과 및 분석
...
| **DecoFlow (우리)** | **90.0±0.5** | 26.6±0.8 | **0.0** | **0.0** | 95.8% |

**ViSA에서 P-AP 격차 분석:**
DecoFlow는 비슷한 I-AUC에도 불구하고 ReplayCAD (41.5%)에 비해 ViSA에서 낮은 P-AP (26.6%)를 달성한다. 이 격차는 세 가지 데이터셋 특화 요인에서 비롯된다:
...
**트레이드오프 해석:**
이는 근본적인 트레이드오프를 나타낸다: DecoFlow는 최대 위치화 정확도보다 **완전 망각 제로**(FM=0.0)를 우선시한다. 엄격한 하위 호환성이 필요한 애플리케이션(예: 인증된 검사 시스템)에서 이 트레이드오프는 유리하다. 위치화를 우선시하는 애플리케이션의 경우, 고해상도 변형(28×28, 4배 파라미터 비용) 또는 도메인별 미세조정이 격차를 메울 수 있다.
...
|------|-------|--------|------|-------|-----|
| Full (DecoFlow) | **97.92** | - | **56.18** | - | **0.0** |
| w/o TAL | 94.97 | -2.95 | 48.61 | **-7.57** | 0.0 |
| w/o GDA | 97.90 | -0.02 | 48.84 | **-7.34** | 0.0 |
| w/o ACL | 92.74 | **-5.18** | 50.06 | -6.12 | 0.0 |
...
2. **GDA**는 분포 이동을 정규화하여 유사하게 +7.34%p P-AP를 기여한다.
3. **ACL**은 I-AUC (+5.18%p)에 필수적이며, 비선형 매니폴드 보정을 통해 학습 안정성을 제공한다.
...
|------|------------|-----------|------|
| **Full (DecoFlow)** | **0.0** | **0.0** | 완전 망각 제로 |
...
|----------|------|--------|-----------|------|------|
| GDA | +7.3%p | +1.2%p | F(1,16)=8.47 | 0.008* | 6.1× |
| TAL | +7.6%p | +3.2%p | F(1,16)=6.23 | 0.021* | 2.4× |
| ACL | +5.2%p | +2.1%p | F(1,16)=5.12 | 0.034* | 2.5× |
| 공간 컨텍스트 | +3.3%p | +3.1%p | F(1,16)=0.89 | 0.356 | 1.1× |
| 스케일 컨텍스트 | +1.7%p | +1.5%p | F(1,16)=1.24 | 0.278 | 1.1× |

**해석:**
세 필수 구성요소(GDA, TAL, ACL) 모두 통계적으로 유의한 상호작용을 보이며 ($p < 0.05$), 고정된 베이스 하에서 기여가 2-6배 증폭된다. 이는 이들이 고정된 베이스 설계의 *특정 한계*를 해결함을 확인한다. 구성요소가 비고정 조건에서도 이점을 제공하지만 (+1.2% ~ +3.2%p), 핵심 통찰은 고정 하에서의 *불균형적 증폭*이다. 공간 및 스케일 컨텍스트는 상호작용을 보이지 않아—고정 여부와 관계없이 모든 AD 시스템에 동등하게 이점을 제공한다.

### 4.8 TAL의 구조적 필요성 심층 분석

Section 4.7에서 TAL이 고정 베이스 조건에서 2.4배 증폭된 효과를 보임을 확인했다. 이 섹션에서는 *왜* TAL이 Decomposed NF에서 구조적으로 필수적인지를 두 가지 분석을 통해 검증한다.
...
**목적:**
TAL이 LoRA의 제한된 용량을 테일 영역 학습에 효율적으로 할당하는지 검증한다. 특히 parameter space(LoRA 가중치)와 feature space(테일 패치) 간의 연결을 명시적으로 분석한다.
...
**Feature-Parameter Space 연결 분석:**

4.6.1에서 식별된 테일 영역(feature space)과 4.6.2의 LoRA 용량 할당(parameter space) 간의 명시적 연결을 검증한다.

**방법:**
- 4.6.1에서 정의된 테일 패치: LL < -5인 패치 (Task 0 베이스 기준)
- LoRA 출력 델타 분석: $\Delta f = f_{\text{base+LoRA}}(x) - f_{\text{base}}(x)$
- 테일 패치에서의 델타 집중도 측정

**표: LoRA 출력 델타의 공간적 분포**

| 조건 | 테일 패치 델타 비율 | 벌크 패치 델타 비율 | 집중도 비 |
|------|-------------------|-------------------|----------|
| TAL 있음 | 68.3% | 31.7% | **2.15** |
| TAL 없음 | 41.2% | 58.8% | 0.70 |

**[그림 6: Feature Space에서 LoRA 델타 분포]**
- (a) TAL 있음: LoRA 델타가 테일 패치(빨간색 영역)에 집중
- (b) TAL 없음: LoRA 델타가 전체 패치에 균등 분산
- (c) 패치별 델타 크기와 4.6.1 테일 비율의 상관관계: TAL 있음 r=0.87, TAL 없음 r=0.23

**핵심 발견:**
4. **Feature-Parameter 정렬:** TAL이 있을 때 LoRA의 출력 변화(델타)가 4.6.1에서 식별된 테일 패치에 2.15배 더 집중된다. 이는 parameter space의 용량 집중(SVD 분석)이 feature space의 테일 영역 보정으로 직접 연결됨을 보여준다.
5. **상관관계 검증:** TAL이 있을 때 패치별 LoRA 델타 크기와 4.6.1의 커버리지 갭 간 상관계수가 0.87로, TAL이 정확히 베이스가 커버하지 못하는 영역을 보정함을 입증한다.

**결론:**
TAL은 단순한 hard example mining이 아니라, Decomposed NF에서 LoRA의 제한된 용량을 테일 학습에 효율적으로 재분배하는 구조적 역할을 수행한다. Feature-parameter space 연결 분석은 이 재분배가 정확히 베이스 NF의 커버리지 갭을 메우는 방향으로 작동함을 입증한다. 이는 TAL이 고정 베이스 + LoRA 아키텍처에서 *대체 불가능한 필수 구성요소*임을 입증한다.
...
- DecoFlow (파란색 실선): 98%에서 평평한 선 (완전 망각 제로)
- 음영 영역은 5개 시드에 대한 ±1 표준편차를 보여줌.

**역방향 및 순방향 전이:**
- **BWT = 0.0:** 이전 태스크에 대한 성능 저하 없음 (Section 4.3에서 상세 검증).
- **FWT:** DecoFlow는 태스크별 어댑터를 사용하므로 직접 적용 불가. 태스크별 학습 전 제로샷 성능은 무작위와 같음 (50% I-AUC).
...
**주요 발견:**
DecoFlow는 3배 적은 GPU 메모리를 사용하고, 태스크당 3배 빠르게 학습하며(Task 0 이후), 리플레이 방법보다 6배 빠르게 추론한다. 태스크당 22-42% 파라미터 오버헤드(ACL 포함 여부에 따라)는 프롬프트 기반 방법보다 높지만, 완전 망각 제로를 보장하는 완전한 격리를 제공한다.
...
DecoFlow를 제시했다. 이는 정규화 흐름의 고유한 구조적 특성을 통해 격리-효율성 딜레마를 해결하는 지속 학습 이상 탐지 프레임워크이다. 임의 함수 속성을 안전한 파라미터 분해를 위한 이론적 기반으로 식별함으로써, 태스크당 22-42% 파라미터 오버헤드만으로 완전한 태스크 격리를 가능하게 했다. 세 가지 필수 구성요소—일반 분포 적응, 테일 인식 손실, 아핀 커플링 레이어—는 상호작용 효과 분석을 통해 고정된 베이스 제약 하에서 증폭된 이점을 제공함이 체계적으로 검증되었다.

DecoFlow는 MVTec-AD에서 최고 성능(98.05% I-AUC, 55.80% P-AP)을 *완전 망각 제로*로 달성하며, 데이터 저장 없이 리플레이 기반 방법을 능가한다.
...
| 순위 | I-AUC | P-AP | LoRA 파라미터 | 태스크당 (ACL 제외) |
|------|-------|------|-------------|------------------|
...
**결론:** 순서에 따른 분산 (σ ≈ 0.05% I-AUC, 0.25% P-AP)이 순서 내 분산과 유사하여, DecoFlow가 태스크 시퀀스에 강건함을 확인한다.
...
모든 DecoFlow vs. 베이스라인 비교는 d > 0.8 (큰 효과)을 보인다.
...
**극심한 분포 이동:**
Task 0 분포에서 >5σ인 특징을 가진 태스크는 GDA 제약 경계에 도달하여, P-AP가 3-5%p 저하된다. 완화: 경계 확장 (안정성 트레이드).

**매우 작은 이상:**
<5 픽셀 (<0.5% 면적) 이상은 14×14 공간 해상도로 인해 낮은 P-AP (<40%)를 보인다. 완화: 4배 파라미터 비용으로 더 높은 해상도 (28×28).

**성능 하한:**
실패 분석을 바탕으로, DecoFlow는 테스트된 모든 구성에서 다음을 보장한다:
- I-AUC ≥ 94%
- P-AP ≥ 45% (MVTec-AD)
- 라우팅 정확도 ≥ 89% (고중첩 시나리오)
