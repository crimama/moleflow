...
\begin{enumerate}
    ...
    \item \textbf{One-stage Task-agnostic 추론}: Mahalanobis 거리 기반 프로토타입 라우터를 통해 추론 시 Task ID 없이 단일 패스로 라우팅과 이상 탐지를 수행한다. 기존 방법의 two-stage 추론 대비 효율적이며, 라우팅 정확도 1.000을 달성하였다.

    \item \textbf{종합적인 실험 검증}: MVTec AD 데이터셋에서 기존 방법 대비 우수한 성능을 달성하였으며 (Image AUROC 0.979, Pixel AP 0.562, Routing Accuracy 1.000), 체계적인 Ablation Study를 통해 각 구성 요소의 유효성을 검증하였다.
\end{enumerate}
