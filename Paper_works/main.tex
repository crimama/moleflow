...
본 섹션에서는 지속적 이상 탐지(Continual Anomaly Detection) 환경에서 파멸적 망각(Catastrophic Forgetting)을 방지하고, 다중 작업(Multi-task)에 대한 정밀한 분포 추정을 수행하기 위한 MoLE-Flow(Mixture of LoRA Experts with Normalizing Flow for Continual Anomaly Detection) 프레임워크를 제안한다.
...
\item \textbf{Prototype-based Router}: 추론 시 작업 ID 없이 자동으로 적절한 전문가를 선택하는 Mahalanobis 거리 기반 라우터
\item \textbf{Tail-Aware Loss}: 분포의 경계를 더 명확하게 학습하도록 유도하는 손실 함수
\end{enumerate}
...
\item Normalizing Flow를 통한 확률 분포 추정:
    ...
    \item Context-Aware Scale Network
    \item LoRA 기반 Coupling Layer
    \item Deep Invertible Adapter
    \end{itemize}
\item Likelihood (손실) 계산
\end{enumerate}
...
이때 중간 층(intermediate layer)의 특징을 추출함으로써 다중 스케일 정보(multi-scale information)를 활용한다.
본 연구에서는 ViT-Base를 백본으로 사용하며, 트랜스포머 블록 $\{1, 3, 5, 11\}$에서 다중 스케일 특징을 집계한다.

\subsubsection{Patch Maker}
추출된 각 특징 맵은 국소 특징을 학습하기 위해 패치 단위로 분할된 후 풀링하여 패치 임베딩을 생성한다.
최종적으로 $\mathbf{F} \in \mathbb{R}^{B \times H \times W \times D}$ 형태의 패치 임베딩이 생성되며, 여기서 $H \times W$는 패치 그리드 크기, $D$는 특징 차원이다.

\subsubsection{Positional Encoding}
Normalizing Flow는 순열 불변성(Permutation Invariance)을 가지므로, 각 패치의 공간적 위치 정보를 보존하기 위해 2D Sinusoidal Positional Encoding $\mathbf{P}$를 패치 임베딩에 추가한다:
\begin{equation}
\mathbf{F}' = \mathbf{F} + \mathbf{P}, \quad \mathbf{P} \in \mathbb{R}^{H \times W \times D}
\end{equation}

\noindent 위치 인코딩은 각 패치의 $(h, w)$ 좌표에 대해 사인-코사인 함수를 적용하여 생성된다.

\subsection{Task Adapters (Whitening)}
\label{sec:adapters}

서로 다른 작업 간의 특징 분포 차이를 정렬하고, Flow 모델의 입력 분포를 안정화하기 위해 Whitening Adapter를 적용한다.

\subsubsection{Task-Agnostic Whitening}

입력 특징 $\mathbf{u}$에 대해 학습 파라미터가 없는 LayerNorm을 적용하여 평균 0, 분산 1의 분포로 정규화(Whitening)한다:
\begin{equation}
...
\noindent 이는 모든 작업이 동일한 통계적 시작점을 갖게 하여, Normalizing Flow를 안정적으로 학습하게 하기 위함이다.

\subsubsection{Task-Specific De-whitening}

정규화된 특징을 해당 작업 고유의 분포로 매핑하기 위해 작업별로 독립된 학습 파라미터 $\gamma_t$와 $\beta_t$를 적용한다.
학습 안정성을 위해 파라미터의 범위를 제한한다:
\begin{align}
\gamma_t &= 0.5 + 1.5 \cdot \sigma(\gamma_{\text{raw}}) \in [0.5, 2.0] \\
\beta_t &= 2.0 \cdot \tanh(\beta_{\text{raw}}) \in [-2.0, 2.0] \\
\hat{\mathbf{F}}_t &= \gamma_t \odot \mathbf{x}_{\text{white}} + \beta_t
\end{align}

\noindent 여기서 $\sigma(\cdot)$는 시그모이드 함수이다.

\subsubsection{Whitening Adapter의 역할}

Whitening Adapter는 다음과 같은 역할을 수행한다:
\begin{itemize}
\item \textbf{입력 표준화}: 새로운 데이터에도 안정적으로 동작하도록 입력을 표준화하여 최적화 난이도를 낮추고 학습 안정성을 확보한다.
\item \textbf{Base Flow 호환성}: Task 0 이후 Base Flow는 동결되므로, 새로운 작업 입력 시 데이터의 분포가 달라지는 경우 모델이 제대로 반응하지 못한다. Whitening Adapter는 모든 데이터를 강제 정규화하여 Base Flow의 최적 동작 범위 내에 들어오도록 보장한다.
\item \textbf{LoRA 역할 분리}: LoRA로 입력되기 전에 스케일 차이를 미리 제거해줌으로써, 제한된 용량의 LoRA가 분포 보정이 아닌 작업 고유 특징 학습에만 집중할 수 있도록 돕는다.
\end{itemize}
...
\subsubsection{Spatial Context Mixer}

기본적인 Normalizing Flow 구조에서는 입력되는 패치 임베딩들을 독립적으로 취급한다.
Positional Encoding을 통해 각 패치의 위치 정보를 반영하고 있으나, 이는 각 패치의 절대적인 위치를 기록한 것일 뿐 패치들 간의 관계에 대해서는 반영하기 어렵다.
즉, 각 패치들의 위치에 대해서는 알 수 있으나, 주변 패치들과 얼마나 다른지 비교하거나 파악하기 어려울 수 있다.

이를 극복하기 위해 Spatial Context Mixer를 도입한다.
이 모듈은 $3 \times 3$ Depthwise Convolution을 통해 인접 패치의 특징 정보를 물리적으로 혼합하고, 학습 가능한 게이트를 통해 문맥 정보의 반영 비율을 적응적으로 조절한다:
\begin{align}
...
\noindent 결과적으로 모델은 단순한 위치별 분포 학습을 넘어, \textbf{주변 문맥과의 상대적 불일치}를 명시적으로 파악할 수 있게 된다.
이는 주변 패턴과의 연속성이 단절되는 형태의 국소적 이상을 탐지하는 데 필수적인 Spatial Inductive Bias를 제공한다.
