...
On MVTec-AD (15 classes, 5 runs), MoLE-Flow achieves $98.03\pm0.19$\% image-level
AUROC and $53.78\pm0.73$\% pixel-level AP with zero forgetting, maintaining 98.7\%
...
The limitations of existing approaches reveal a fundamental structural constraint in continual learning: \textbf{parameter isolation} and \textbf{parameter efficiency} appear mutually exclusive.

\begin{principle}[Isolation-Efficiency Dilemma]
\label{principle:dilemma}
For shared-weight models with parameters $\theta$, any gradient update $\theta \leftarrow \theta - \eta \nabla \mathcal{L}_t(\theta)$ on task $t$ affects outputs for all previous tasks, causing forgetting by definition. Complete isolation (FM $= 0$) requires task-specific parameters $\{\theta_t\}_{t=0}^{T-1}$, incurring $O(T \times |\theta|)$ memory cost.
\end{principle}

Table~\ref{tab:pilot_dilemma} empirically validates this dilemma on MVTec-AD with 5 sequential tasks:

\begin{table}[h]
\centering
\small
\caption{The Isolation-Efficiency Dilemma: Existing methods cannot achieve zero forgetting (FM=0) with practical memory overhead. Pilot experiment on MVTec-AD (5 classes).}
\label{tab:pilot_dilemma}
\begin{tabular}{lccccc}
\toprule
\textbf{Method} & \textbf{I-AUC} & \textbf{FM} & \textbf{Memory/Task} & \textbf{Zero FM?} \\
\midrule
...
\paragraph{Why Existing Methods Cannot Achieve Zero Forgetting.}
\begin{itemize}
    \item \textbf{Regularization (EWC, MAS)}: The constraint $\lambda \sum_i F_i(\theta_i - \theta_i^*)^2$ limits but cannot eliminate parameter drift. Forgetting is \emph{mitigated}, not \emph{prevented}.
    \item \textbf{Replay}: With buffer size $K$ and $T$ tasks, each task receives $K/T$ replay samples. As $T \to \infty$, replay density vanishes.
    \item \textbf{Architecture (PackNet)}: Binary masks provide isolation but suffer from capacity saturation as $T$ grows.
\end{itemize}

\paragraph{Our Insight: Resolving the Dilemma via Structural Properties.}
The dilemma assumes that isolation requires full parameter copies. We observe that \textbf{Normalizing Flow coupling layers} possess a unique structural property---the \emph{Arbitrary Function Property}---that enables parameter decomposition without violating model validity. This allows us to achieve:
\begin{equation}
\mathbf{W}_{\text{task}} = \mathbf{W}_{\text{shared}} + \Delta\mathbf{W}_{\text{task}}, \quad |\Delta\mathbf{W}| \ll |\mathbf{W}|
\end{equation}
where $\mathbf{W}_{\text{shared}}$ is frozen (isolation) and $\Delta\mathbf{W}_{\text{task}}$ is low-rank (efficiency), achieving both constraints simultaneously.
...
\begin{enumerate}
    ...
    \item \textbf{Comprehensive Experimental Validation}: On MVTec-AD (15 classes, 5 independent runs), MoLE-Flow achieves $98.03\pm0.19$\% image-level AUROC and $53.78\pm0.73$\% pixel-level AP with zero forgetting, maintaining 98.7\% of single-task oracle performance with 100\% routing accuracy.
\end{enumerate}
...
Building on Principle~\ref{principle:dilemma}, MoLE-Flow (Mixture of LoRA Experts for Normalizing Flow) instantiates parameter decomposition for continual anomaly detection. The framework comprises a feature extractor, MoLE-based normalizing flow, task-specific adapters, and a prototype-based router.

As established in Section~\ref{sec:introduction}, the Arbitrary Function Property of NF coupling layers enables decomposing subnets as:
\begin{equation}
    \text{MoLESubnet}(\mathbf{x}) = \text{MLPBase}(\mathbf{x}; \Theta_{\text{base}}) + \frac{\alpha}{r}\mathbf{B}_t(\mathbf{A}_t \mathbf{x}),
\end{equation}
where $\Theta_{\text{base}}$ is frozen after Task 0 and $\{\mathbf{A}_t, \mathbf{B}_t\}$ are task-specific low-rank adapters. This achieves complete \textbf{parameter isolation} with only $O(N \times \text{rank})$ memory scaling.
...
The frozen-base design achieves zero forgetting but introduces structural rigidity. We identify three \textbf{integral components}---Whitening Adapter (WA), Tail-Aware Loss (TAL), and Deep Invertible Adapter (DIA)---that are not generic performance boosters but \emph{necessary compensations} for specific consequences of the frozen-base paradigm. Section~\ref{sec:interaction_effect} validates this distinction through Interaction Effect Analysis.

\subsubsection{Whitening Adapter (Distribution Alignment)}
\label{sec:adapters}

\paragraph{Problem: Input Interface Mismatch.}
\begin{itemize}
...
\paragraph{Solution: Distribution Alignment.}
Whitening Adapter aligns input distributions to the frozen base's expected input range:
\begin{itemize}
...
\subsubsection{Deep Invertible Adapter (Nonlinear Manifold Correction)}
\label{sec:dia}

\paragraph{Problem: Global Transformation Rigidity.}
The frozen base provides a fixed global transformation structure. While LoRA adds task-specific linear corrections, complex manifold differences between tasks require nonlinear adaptation.

\paragraph{Solution: Task-Specific Invertible Correction.}
\begin{itemize}
...
\paragraph{Problem: Gradient Concentration at Bulk.}
When the base is frozen, LoRA provides only ``slight corrections'' to the base transformation. Standard NLL training causes gradients to concentrate on bulk (easy) patches, leaving tail (hard) patches under-optimized---precisely where anomaly detection decisions are most critical.

\paragraph{Solution: Tail-Focused Training.}
\begin{itemize}
