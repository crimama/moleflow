...
\begin{itemize}
    \item Normalizing Flow는 입력 분포의 변화에 매우 민감하여, 분포가 달라질 때 학습이 불안정하거나 성능 저하가 두드러지는 단점이 있다.
    \item 이러한 NF의 불안정성을 완화하고, 서로 다른 작업 간의 특징 분포 차이를 정렬하여 입력 분포를 안정화하기 위해 Whitening Adapter를 적용한다.
\end{itemize}
