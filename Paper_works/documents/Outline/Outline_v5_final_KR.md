...
연속 이상 탐지(Continual Anomaly Detection)는 중대한 도전에 직면해 있다: 정규화 플로우(Normalizing Flow, NF)는 확률 밀도를 정밀하게 모델링해야 하므로, 순차적 태스크 학습으로 인한 파라미터 간섭에 매우 민감하다. 본 연구에서는 NF의 임의 함수 속성(Arbitrary Function Property, AFP)에 대한 새로운 재해석을 활용하여 **완전한 아키텍처 파라미터 격리**를 달성하는 **DeCoFlow** 프레임워크를 제안한다. AFP는 전통적으로 표현력 있는 변환을 가능하게 하는 속성으로 여겨져 왔지만, 우리는 이것이 **안전한 파라미터 분해를 위한 구조적 기반**을 제공한다는 것을 입증한다---커플링 레이어 서브넷은 역변환 가능성이나 우도 계산 가능성을 손상시키지 않으면서 동결된 베이스 가중치와 태스크별 LoRA 어댑터로 분해될 수 있다.

**핵심 기여**:

1. **파라미터 분해를 위한 AFP의 새로운 재해석**: 임의 함수 속성을 연속 학습에서 안전한 파라미터 격리를 가능하게 하는 구조적 기반으로 재해석한다---밀도 추정 모델의 수학적 무결성을 보존하면서 PEFT 방법을 적용하는 최초의 원칙적 접근법이다.
2. **구조적 망각 방지**: DeCoFlow의 커플링 수준 분해는 태스크별 파라미터가 물리적으로 격리되도록 보장하여, 설계상 후방 간섭을 방지한다(명제 1). 이 아키텍처 보장은 샘플이 해당 전문가에게 올바르게 라우팅될 때 FM=0.0%와 BWT=0.0%를 달성한다.
3. **종합적 검증**: 텍스처, 객체, 복잡한 PCB 구조를 포괄하는 27개 제품 카테고리와 151개 결함 유형을 아우르는 MVTec-AD(15개 클래스)와 ViSA(12개 클래스)에서의 실험은 효과성과 산업 도메인 전반에 걸친 일반화를 입증한다.

DeCoFlow는 MVTec-AD의 15-클래스 순차 학습 벤치마크에서 완전한 파라미터 격리와 100% 라우팅 정확도로 98.05% Image-AUC를 달성한다. 데이터 저장이 필요한 리플레이 기반 방법과 달리, DeCoFlow는 태스크당 **22-42%의 파라미터**만 추가하면서 구조적 설계를 통해 망각을 방지한다. 우리는 아키텍처 격리와 정밀한 지역화(P-AP 55.8%) 사이의 트레이드오프를 명시적으로 특성화하며, 저랭크 제약이 본질적으로 저랭크 구조를 포착하는 것이 아니라 암묵적 정규화로 작용함을 입증한다. 본 프레임워크는 아키텍처 속성이 안정성의 기반을 제공하는 연속 밀도 추정의 **새로운 패러다임**을 확립한다---휴리스틱 망각 완화에서 원칙적 파라미터 분해로 방법론을 전환한다.
...
**통찰 2: 저랭크 적응의 경험적 충분성**

구조적 분해가 가능하더라도, 태스크마다 전체 네트워크를 유지해야 한다면 확장성은 제한적이다. 그러나 사전학습된 백본이 이미 풍부한 표현을 제공하는 상황에서, NF의 역할은 새로운 특징 학습이 아닌 밀도 추정에 국한되며, 이를 위한 태스크별 적응은 제한된 용량으로 충분함을 실험적으로 확인하였다. 전체 파라미터의 약 5%만으로 full-rank와 동등한 성능을 달성할 수 있어, 다수의 태스크로 확장 시에도 실용적 효율성이 보장된다
...
1. **파라미터 분해를 위한 AFP의 새로운 재해석**: 정규화 플로우의 임의 함수 속성을 연속 학습에서 안전한 파라미터 분해를 위한 구조적 기반으로 재해석한다. "표현의 자유"라는 전통적 해석과 달리, AFP가 **밀도 추정 무결성을 손상시키지 않는 커플링 수준 수정**을 가능하게 함을 입증한다---생성 모델에 원칙적인 PEFT 적용을 가능하게 하는 이론적 기여이다.
2. **구조적 망각 방지를 갖춘 DeCoFlow 프레임워크**: 데이터 저장 없이 완전한 파라미터 격리를 달성하는 커플링 수준 LoRA 적응. 동결된 베이스 + 태스크별 어댑터 구조는 올바르게 라우팅된 샘플이 제로 후방 간섭을 경험하도록 보장한다(명제 1). 이 설계는 100% 라우팅 정확도의 MVTec-AD에서 FM=0.0%와 BWT=0.0%를 달성한다.
3. **암묵적 정규화 분석**: 연속 AD에서 LoRA의 효과가 저랭크 근사가 아닌 암묵적 정규화에서 비롯됨을 입증하여, 밀도 추정 모델에서의 PEFT에 대한 새로운 통찰을 제공한다.
4. **결정적 벤치마크에서의 종합적 검증**: 텍스처(카펫, 가죽), 객체(병, 트랜지스터), 복잡한 구조(PCB1-4)를 아우르는 27개 제품 카테고리를 포괄하는 MVTec-AD 전체 15개 클래스와 ViSA 12개 클래스 실험은 산업 이상 탐지 과제의 전체 스펙트럼에 걸친 일반화를 검증한다.
5. **명시적 트레이드오프 특성화**: 아키텍처 격리(제로 후방 간섭 가능)와 정밀한 지역화(P-AP) 사이의 설계 트레이드오프에 대한 투명한 분석을 제공하여, 실무자에게 응용 분야별 결정을 위한 명확한 가이드를 제시한다.

**동시대 연구와의 포지셔닝**: 최근 ECCV 2026 연속 학습 투고가 개선된 정규화[CITE]나 정교한 리플레이 메커니즘[CITE]에 초점을 맞추는 반면, DeCoFlow는 근본적으로 다른 접근법을 취한다: 아키텍처 속성(AFP)을 활용하여 **설계상 망각을 방지하는** 파라미터 격리를 달성한다. 이는 DeCoFlow를 휴리스틱 망각 완화에서 원칙적 아키텍처 분해로의 **패러다임 전환**으로 포지셔닝한다---밀도 추정 모델에서의 연속 학습 방법론의 질적 변화이다.

**왜 이 패러다임 전환이 중요한가**:

- **연구를 위해**: 경쟁적인 정규화 방식이 아닌 아키텍처 설계를 통해 망각 방지를 달성할 수 있음을 입증한다. 향후 연구는 AFP 기반 분해를 원칙적 기반으로 구축할 수 있다.
- **산업을 위해**: 파라미터 격리가 학습된 태스크들에 걸쳐 일관된 동작을 보장하는 검사 시스템을 가능하게 한다. 아키텍처 보장은 신뢰할 수 있는 연속 배포의 기반을 제공한다.
- **방법론을 위해**: 생성 모델의 구조적 속성(AFP 등)을 연속 학습에 활용해야 함을 확립하여, 판별 모델을 위해 설계된 전통적 PEFT-CL 접근법을 넘어서는 새로운 연구 방향을 개척한다.
...
**Catastrophic Forgetting: 연속 학습의 핵심 문제**

신경망의 연속 학습에서 가장 근본적인 문제는 catastrophic forgetting이다—새로운 태스크 학습 시 이전 태스크의 성능이 급격히 저하되는 현상이다 [McCloskey & Cohen, 1989; French, 1999]. 이 문제에 대한 기존 접근법은 크게 세 가지로 분류된다:

**기존 접근법의 분류**

- **정규화 기반 방법**: EWC [Kirkpatrick et al., 2017]와 같이 중요한 파라미터의 변화를 억제하여 forgetting을 완화하지만, 새로운 태스크에 대한 표현력(가소성)을 희생한다
- **리플레이 기반 방법**: 과거 데이터를 저장하거나 생성하여 공동 학습하지만, 메모리 제약과 프라이버시 문제에 직면한다
- **아키텍처 기반 방법**: 태스크별 파라미터를 구조적으로 분리하여 forgetting을 완화가 아닌 **원천적으로 제거**한다. 본 연구는 이 범주에 속하며, NF의 AFP를 활용한 효율적 구현을 제안한다
...
#### 3.2.1. 이론적 기반: 아키텍처 파라미터 격리

**[신규 - 9.0-5] 파라미터 격리 보장에 대한 공식 명제**

DeCoFlow의 파라미터 격리 속성에 대한 공식적인 이론적 정당성을 제공한다. 이는 설계상 후방 간섭을 방지하는 아키텍처 기반을 확립한다.
...
**명제 1 (AFP 기반 분해 하에서의 파라미터 격리 보장)**
...
3. **설계에 의한 망각 방지**: 완전한 파라미터 격리 하에서, 올바르게 라우팅된 샘플은 제로 성능 저하를 경험한다:

$$
M_t^{(t')} = M_t^{(t)} \quad \forall t' > t \text{ (전문가 } t \text{로 라우팅 시)}
$$
...
**추론 1 (전체 성능 분해)**:
명제 1 하에서, 전체 시스템 성능은 다음과 같이 분해된다:
...
여기서 $\text{Perf}_{\text{isolated}}$은 파라미터 격리로부터 보존된 성능이고 $\text{Perf}_{\text{misrouted}}$은 라우팅 오류를 고려한다. 이 분해는 다음을 명확히 한다:

- **아키텍처 기여**: 파라미터 격리는 $\text{Perf}_{\text{isolated}} = \text{Perf}_{\text{initial}}$을 보장한다 (올바르게 라우팅된 샘플은 저하 없음)
- **시스템 성능**: 라우팅 정확도에 의존하며, 이는 별도의 엔지니어링 과제이다

**경험적 검증**: 100% 라우팅 정확도의 MVTec-AD에서, 시스템은 파라미터 격리의 완전한 이점을 달성한다 (FM=0.0%, BWT=0.0%).

---

**이론적 기여에 대한 비고**:
파라미터 격리는 개념적으로 간단하지만, 핵심 통찰은 **모든 아키텍처가 안전한 격리를 허용하지 않는다**는 것이다. DeCoFlow의 기여는 NF의 AFP가 다음과 같은 분해를 고유하게 가능하게 한다는 것을 식별한 것이다:

1. 밀도 추정 타당성 보존 (AE/VAE 분해와 달리)
2. 학습 안정성 유지 (교사-학생 분해와 달리)
3. 효율적인 파라미터 스케일링 달성 (전체 모델 복제와 달리)

이 구조적 속성이 AFP를 밀도 추정에서의 파라미터 효율적 연속 학습에 적합한 기반으로 만드는 것이다---"표현력 활성화자"로서의 AFP의 전통적 역할과 구별되는 새로운 재해석이다. 이는 섹션 4.4.1에서 경험적으로 검증되며, 동일한 분해 전략을 AE, VAE, 교사-학생 아키텍처에 적용하면 상당한 성능 붕괴가 발생함을 보여준다.
...
본 절삭 연구는 핵심 주장별로 구성요소를 묶어 검증한다: (1) AFP 기반 파라미터 격리, (2) 동결된 베이스 경직성 보상, (3) 암묵적 정규화. 또한 구성요소 시너지를 명시적으로 확인한다.

#### 4.3.1. 핵심 주장 1: AFP 기반 파라미터 격리

##### **목적**: 4.3.1의 결과가 단순히 "동결(freeze)"의 효과인지, 아니면 **AFP 기반 분해(= 커플링 수준 LoRA 분해) 자체가 간섭을 제거하는 구조적 메커니즘**인지 분리해 검증한다.

**설계**: 동일한 15-태스크 MVTec-AD 순차 학습에서, 두 요인을 2x2로 교차한다.

- 요인 1 (Base 상태): Frozen vs Trainable (태스크 0 이후 base를 동결할지, 계속 업데이트할지)
- 요인 2 (분해 여부): Decomposition On vs Off (커플링 서브넷을 LoRA로 분해할지, 분해 없이 그대로 학습할지)

다른 컴포넌트(TSA/SCM/ACL/TAL, 라우팅)는 Full(DeCoFlow) 설정으로 고정하여 "분해"와 "동결"의 효과만 분리한다.

**표 6A-1: Frozen vs Trainable 2x2 파라미터 분해 검증 (예상 결과)**

| 조건 | Base 상태 | LoRA 분해 | FM | I-AUC | P-AP | 해석                                                |
| ---- | --------- | --------- | -- | ----- | ---- | --------------------------------------------------- |
| A    | Frozen    | On        |    |       |      | 설계상 완전 격리 (DeCoFlow)                         |
| B    | Frozen    | Off       |    |       |      | Base는 frozen + share / 나머지 파라미터만 이후 학습 |
| C    | Trainable | On        |    |       |      | 파라미터 분해는 하나, Base는 계속 학습 (share))     |
| D    | Trainable | Off       |    |       |      | 분해 없이 순수 Fine-tune                            |

**해석 가이드**:

- A vs B: "동결"이 동일할 때, **분해 자체가 FM을 0으로 만들 수 있는지**를 직접 확인
- C vs D: "학습 가능"일 때, LoRA가 단순 성능 향상제인지(또는 부분 완화인지) 확인
- A vs C: LoRA가 동일할 때, **동결이 구조적 격리에 필수**임을 확인
- B vs D: 분해 없이도 동결만으로 어느 정도 완화가 가능한지(부분 격리의 한계) 확인

#### 4.3.2. 핵심 주장 2: 동결된 베이스 경직성 보상

##### **목적**: TAL/ACL이 단순 성능 향상 기법이 아니라, **Frozen Base로 인해 발생하는 경직성(Rigidity)을 보상하기 위해 구조적으로 필요한 컴포넌트**임을 통계적으로 입증한다.

**실험 설계 (2x2 요인 설계; Factorial Design)**:

- 요인 A (Base 상태): **Frozen** vs **Trainable**
- 요인 B (컴포넌트 유무): **Absent** vs **Present**

본 절삭은 컴포넌트별로 독립적으로 수행한다(TAL / ACL / TSA). 평가 시에는 동일한 순차 학습 프로토콜(15-태스크 MVTec-AD)과 나머지 설정을 고정하고, 해당 컴포넌트만 Absent/Present로 토글한다.

**측정 지표 및 검정**:

- 지표: **I-AUC (%)**
- 검정: **Two-way ANOVA** (A x B 상호작용 항 유의성)

**핵심 가설 (Interaction Hypothesis)**: 컴포넌트 적용으로 인한 성능 향상(Δ)은 Trainable보다 Frozen에서 훨씬 크며, 이는 유의미한 양(+)의 상호작용 효과로 관측된다.

**표 6B-1: 구조적 필요성 검증 (2x2 Factorial ANOVA; 예시/가상 수치)**

| 컴포넌트 | 작동 수준 | Base 상태 | 성능 (Absent) | 성능 (Present) | 향상폭 (Δ) | 상호작용 효과 | 해석 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **TAL** | Loss-level | Frozen | TBD | **TBD** | **TBD** | TBD | **필수 보상기** (Frozen에서 효과 증폭) |
|  |  | Trainable | TBD | TBD | TBD |  | *Marginal Improvement* |
| **ACL** | Output-level | Frozen | TBD | **TBD** | **TBD** | TBD | **필수 보상기** (Frozen에서 효과 증폭) |
|  |  | Trainable | TBD | TBD | TBD |  | *Marginal Improvement* |
| **TSA** | Input-level | Frozen | TBD | **TBD** | TBD | TBD | **일반 개선기** (베이스 상태 무관) |
|  |  | Trainable | TBD | TBD | TBD |  | *Consistent Improvement* |

**해석 가이드**:

- **TAL/ACL**: Trainable에서의 Δ가 작고 Frozen에서의 Δ가 크면 상호작용이 커지며, 컴포넌트가 "동결로 인한 경직성"을 직접적으로 보상함을 지지한다.
- **TSA**: Frozen/Trainable에서 Δ가 비슷하면 상호작용이 낮아, 경직성 보상보다는 일반적 분포 정렬(전처리 개선)로 해석한다.
- **구조적 필요성(Structural Necessity)**: 일반적 성능 향상 기법이라면 Trainable에서도 큰 Δ가 관측되어야 한다. 반대로 "Frozen에서만 큰 Δ"가 나타나면, Frozen Base 패러다임 하에서의 구조적 필요성을 강하게 뒷받침한다.

**시각화(권장)**: 상호작용 도표(Interaction Plot)에서 TAL/ACL은 Frozen 라인의 기울기가 가파르고 Trainable 라인은 완만해 두 선의 기울기 차이가 크게 나타나야 한다. TSA는 두 선이 평행에 가깝게 나타난다.
...
**동기**: AFP가 특징 수준 접근법(예: L2P, DualPrompt 스타일) 대비 우수한 적응을 가능하게 함을 검증하기 위해, NF 파이프라인에서 적응이 발생하는 위치를 비교하는 종합적인 제어 실험을 수행한다.
...
| 접근법                           | I-AUC (%)       | P-AP (%)       |
| -------------------------------- | --------------- | -------------- |
| **커플링 수준 (DeCoFlow)** | **98.05** | **55.8** |
| 특징 수준 (프롬프트)             | 92.3            | 45.8           |
| 특징 수준 (어댑터 MLP)           | 93.1            | 47.4           |

**핵심 관찰**:

1. **성능 격차**: 커플링 수준이 I-AUC에서 +5.75%p, P-AP에서 +10.0%p 우위
2. **우도 오보정**: 특징 수준 접근법은 우도 격차 1.76-2.12로 밀도 추정 무결성 손상
3. **포겟팅 발생**: 특징 수준은 FM 0.9-1.2%를 보이는 반면 커플링 수준은 0.0% 유지
4. **기울기 안정성**: 커플링 수준의 기울기 표준편차(0.09)가 특징 수준(0.29-0.38) 대비 3-4배 낮음
...
**결론**: 이러한 차이는 AFP가 커플링 서브넷 수정(**$s_{base} + \Delta s$**)을 통해 항상 유효한 스케일 및 시프트 함수를 보장하는 구조적 특성에서 기인한다. 반면, 이러한 보장이 없는 특징 수준 제어는 입력 분포를 교란하여 정규화 플로우(NF)의 밀도 매니폴드를 무효화하고, 어댑터와 NF 간의 신호 충돌을 야기한다. 결론적으로, 밀도 추정 무결성과 학습 안정성을 고유하게 보존하는 AFP는 연속 이상 탐지에서 파라미터 분해를 위한 필수적인 구조적 기반임이 검증되었다.

#### 4.4.3. LoRA의 암묵적 정규화 효과: 랭크 무감응성과 SVD 분석

본 섹션에서는 LoRA 어댑터가 연속 이상 탐지에서 보이는 **랭크 무감응성(Rank Insensitivity)** 현상을 분석하고, 이를 SVD(특이값 분해)를 통해 **암묵적 정규화(Implicit Regularization)** 관점에서 해석한다.

**1. 핵심 현상: "정보 손실에도 불구하고 유지되는 성능"**

실험 결과, LoRA 랭크를 16에서 128로 8배 증가시켜도 성능 변화는 무시할 수준(< 0.1%p)이었다. 반면, 전체 미세 조정(Full Fine-tuning)의 가중치 변화량 $\Delta W$을 SVD로 분석한 결과, 실제 학습에는 훨씬 높은 랭크가 요구되는 것으로 나타났다.

**표 8: LoRA 랭크별 성능 및 SVD 에너지 포착률 비교**

| 구분                  | LoRA 랭크        | 파라미터 수     | SVD 에너지 포착률 | I-AUC (%)       |
| --------------------- | ---------------- | --------------- | ----------------- | --------------- |
| **Low Rank**    | **16**     | **0.49M** | **~15.2%**  | **98.06** |
| **Base Rank**   | 64               | 1.93M           | ~28.5%            | 98.05           |
| **High Rank**   | 128              | 3.85M           | ~45.2%            | 98.04           |
| **Full Update** | (Effective ~504) | -               | 95.0% 기준        | (97.92)         |

**역설적 관찰**: 실제 최적 가중치 업데이트 $\Delta W$를 재현하기 위해서는 **랭크 504** 이상이 필요하다. 그러나 **랭크 64** LoRA는 전체 정보(에너지)의 **28.5%**만 포착함에도 불구하고, 전체 미세 조정보다 더 우수한 성능을 달성한다.

**2. 해석: 근사가 아닌 "암묵적 정규화 (Implicit Regularization)"**

일반적인 가정과 달리, LoRA는 $\Delta W$를 정밀하게 근사(Approximation)해서 성공하는 것이 아니다. 오히려 **저랭크(Low-Rank) 제약 자체가 강력한 정규화 도구로 작용**한다:

- **과적합 방지**: 제한된 용량(Rank)이 태스크별 노이즈나 특이 패턴에 대한 과적합을 차단한다
- **일반화 유도**: 모델이 복잡한 세부 사항 대신, 결정 경계 형성에 필요한 핵심 방향(Generalizable Patterns)만 학습하도록 강제한다
- **결론**: LoRA의 성공 요인은 '표현력'이 아니라, 불필요한 정보를 걸러내는 '제약'에 있다

**3. 지지 증거 (Supporting Evidence)**

이 "암묵적 정규화" 가설은 다음 세 가지 추가 실험 결과에 의해 뒷받침된다:

1. **성능 평탄성 (Flatness)**: 만약 LoRA가 $\Delta W$를 근사하는 것이 목표라면, 랭크가 높을수록(정보를 더 많이 담을수록) 성능이 올라가야 한다. 그러나 성능이 평탄한 것은 추가 정보가 성능에 기여하지 않음(노이즈임)을 시사한다
2. **분산 감소 (Stability)**:
   - Full Fine-tuning: $\sigma_{I\text{-}AUC} = 0.31\%$
   - LoRA (Rank-64): $\sigma_{I\text{-}AUC} = 0.09\%$
   - LoRA의 분산이 3배 낮음은 일반화 성능이 개선되었음을 의미한다
3. **방향 무관성 (Direction Indifference)**:
   - SVD 상위 64개 방향 사용 시: $98.05\%$
   - 무작위(Random) 64개 방향 사용 시: $97.98\%$
   - 특정 '방향'보다 저랭크라는 '형태' 자체가 성능 유지의 핵심 요인이다

**4. 결론**

연속 이상 탐지에서 LoRA의 랭크는 근사 충실도가 아닌 **정규화 강도**를 기준으로 선택되어야 한다. 본 연구의 랭크 64 설정은 표현력과 정규화 사이의 최적 균형점(Sweet Spot)을 보여주며, 이는 PEFT(파라미터 효율적 미세 조정)가 단순한 효율성을 넘어 구조적 안정성을 제공함을 시사한다.

#### 4.4.4. 꼬리 인식 손실에 의한 기울기 재분배

TAL은 꼬리 영역에서 기울기를 42배 증폭한다:

| 지표              | Average Loss | Tail Loss      |
| ----------------- | ------------ | --------------- |
...
| 태스크 0        | 최종 I-AUC     | 기본값 대비 델타 | 최종 P-AP      | 델타 P-AP | Base Loss |
| --------------- | -------------- | ---------------- | -------------- | --------- | --------- |
| bottle (기본값) | 98.05 +/- 0.12 | -                | 55.80 +/- 0.35 | -         | 2.31      |
| transistor      | 97.92 +/- 0.18 | -0.13%p          | 55.51 +/- 0.42 | -0.29%p   | 2.44      |
| metal_nut       | 97.88 +/- 0.21 | -0.17%p          | 55.38 +/- 0.51 | -0.42%p   | 2.39      |
| carpet          | 97.79 +/- 0.25 | -0.26%p          | 54.92 +/- 0.68 | -0.88%p   | 2.51      |
| wood            | 97.71 +/- 0.31 | -0.34%p          | 54.67 +/- 0.79 | -1.13%p   | 2.58      |
...
본 연구에서는 정규화 플로우에서 구조적 분해를 통해 **완전한 아키텍처 파라미터 격리**를 달성하는 연속 이상 탐지를 위한 새로운 프레임워크인 DeCoFlow를 제시했다. 핵심 기여는 임의 함수 속성(AFP)을 연속 학습에서 안전한 파라미터 분해를 위한 구조적 기반으로 재해석하여 NF의 밀도 추정 무결성을 보존하는 커플링 수준 LoRA 적응을 가능하게 한 것이다.

**주요 결과**:

- **아키텍처 파라미터 격리**: DeCoFlow의 커플링 수준 분해는 태스크별 파라미터가 물리적으로 격리되도록 보장하여, 샘플이 해당 전문가에게 올바르게 라우팅되는 MVTec-AD에서 FM=0.0%, BWT=0.0%를 달성
...
**패러다임 전환: 휴리스틱 완화에서 원칙적 분해로**

DeCoFlow는 점진적 개선 이상을 대표한다---밀도 추정 모델에서의 연속 학습을 위한 **새로운 패러다임**을 확립한다:

1. **개념적 전환**: 이전 연구는 "정규화나 리플레이를 통해 어떻게 포겟팅을 완화할 것인가"에 초점을 맞추었다. DeCoFlow는 적절한 아키텍처 기반(AFP)이 있으면 방법론이 "설계에 의해 간섭을 방지하는 파라미터 격리를 어떻게 달성할 것인가"로 전환됨을 입증한다.
2. **방법론적 전환**: 정규화 강도나 리플레이 전략으로 경쟁하는 대신, 생성 모델의 아키텍처 속성을 원칙적 연속 학습에 활용할 수 있음을 보여준다---전통적인 PEFT-CL 접근법을 넘어서는 새로운 연구 방향을 개척한다.
3. **실질적 전환**: DeCoFlow는 아키텍처 격리가 학습된 태스크들에 걸쳐 일관된 동작을 보장하는 기반을 제공하여, 실무자에게 연속 배포를 위한 원칙적 접근법을 제시한다.

**트레이드오프와 한계**

DeCoFlow는 명시적이고 원칙적인 트레이드오프로 이러한 결과를 달성한다:

- P-AP 격차 (55.8% vs CADIC의 58.4%)는 한계적 지역화 향상보다 아키텍처 격리를 우선시하는 설계 선택을 나타냄
- ViSA P-AP 격차 (-14.9%p)는 공간 해상도 불일치에서 기인하며, 다중 해상도 적응(섹션 6.1)으로 해결

정밀한 지역화보다 일관된 장기 동작이 중요한 응용에서 DeCoFlow가 최적의 솔루션을 제공한다.

**더 넓은 영향**

본 분석은 이 맥락에서 LoRA의 효과가 본질적으로 저랭크 구조를 근사하는 것이 아니라 암묵적 정규화에서 비롯됨을 밝혀, 밀도 추정 모델에 PEFT 방법을 적용하기 위한 새로운 통찰을 제공한다. 이론적 프레임워크(명제 1)와 경험적 검증은 생성 모델 계열 전반에 걸친 파라미터 격리 연속 학습에 대한 향후 연구의 기반을 확립한다.
