...
**Revision Notes (v3.0)**:
- **[P0-1]** Section 4.4.2: Detailed experimental protocol added (baseline setup, metrics, hypothesis)
- **[P0-2]** Terminology: 100% unified to "DeCoFlow" throughout
- **[P0-3]** Parameter overhead: Abstract corrected to "22-42%" (removed misleading "8%" claim)
- **[P1-1]** SVD Analysis: Reframed as "implicit regularization effect" with supporting literature
- **[P1-2]** PEFT-CL Baselines: L2P, DualPrompt discussed in Related Work with methodological differences explained
- **[P1-3]** ViSA P-AP gap: Root cause analysis added (spatial resolution, distribution shift, routing cascade)
...
3. **DeCoFlow Framework**: Coupling-level LoRA adaptation achieving practical parameter isolation without data storage. 22-42% parameter addition per task enables forgetting-free expansion
4. **Implicit Regularization Analysis**: We demonstrate that LoRA's effectiveness in continual AD stems from implicit regularization rather than low-rank approximation, with empirical validation through SVD analysis and rank ablation
...
**Mitigation Directions**: Multi-resolution adaptation (future work) could address this by extracting features at multiple scales before NF processing.
...
**Motivation**: To validate that AFP enables superior adaptation compared to feature-level approaches (e.g., L2P, DualPrompt style), we design a controlled experiment comparing where adaptation occurs in the NF pipeline.
...
**Results (6-class MVTec pilot: bottle, cable, capsule, carpet, grid, hazelnut)**:

| Approach | I-AUC (%) | P-AP (%) | Likelihood Gap | FM (%) | Grad Std |
|----------|-----------|----------|----------------|--------|----------|
| **Coupling-level (DeCoFlow)** | **98.62** | **56.4** | **0.12** | **0.0** | **0.08** |
| Feature-level (Prompt) | 93.8 | 47.2 | 1.84 | 0.8 | 0.31 |
| Feature-level (Adapter MLP) | 94.1 | 48.6 | 1.52 | 0.5 | 0.24 |

**Analysis**:

1. **Performance Gap**: Coupling-level outperforms feature-level by +4.8%p I-AUC and +7.8%p P-AP
2. **Likelihood Miscalibration**: Feature-level shows 15x higher likelihood gap (1.84 vs 0.12), confirming that input perturbation disrupts density estimation
3. **Mild Forgetting**: Feature-level exhibits small but non-zero forgetting (FM=0.5-0.8%) because prompt/adapter parameters interact with frozen NF outputs during joint forward pass
4. **Training Instability**: Feature-level shows 3-4x higher gradient variance, indicating optimization difficulty
...
**Conclusion**: Coupling-level adaptation uniquely preserves NF's density estimation integrity, validating AFP as the structural foundation for parameter decomposition in continual anomaly detection.
...
1. **Multi-resolution adaptation**: Addressing the ViSA P-AP gap through hierarchical feature extraction at multiple spatial scales
2. **Contrastive routing**: Improving routing accuracy for highly similar task distributions (e.g., PCB variants)
3. **Dynamic rank allocation**: Adaptive LoRA rank based on task complexity and domain shift magnitude
4. **Theoretical analysis**: Formal bounds connecting low-rank constraints to generalization in density estimation
