...
**Revision Notes (v2.0)**:
- Terminology unified to "DeCoFlow" and "Arbitrary Function Property (AFP)"
- C2 claim revised based on SVD analysis (selective learning of critical singular directions)
- P-AP performance gap analysis added (explicit trade-off discussion)
- Section 4 placeholders filled with actual experimental results
- Section 4.4.2 experiment description added
- AFP novelty strengthened
...
(To be written based on the following key points)
- **Problem**: Catastrophic forgetting in continual anomaly detection with Normalizing Flows
- **Key Insight**: AFP enables safe parameter decomposition in NF coupling layers
- **Method**: DeCoFlow - coupling-level LoRA decomposition with frozen base
- **Results**: 98.05% I-AUC, **Zero Forgetting (BWT=0.0, FM=0.0)**, 100% routing accuracy
- **Trade-off**: Explicit trade-off between zero forgetting and fine-grained localization (P-AP 55.8%)
...
**Insight 2: LoRA Selectively Learns Critical Singular Directions (Empirical Insight)**
...
**Revised Claim Based on SVD Analysis**:
- SVD analysis of weight updates $\Delta W$ reveals that the effective rank capturing 95% energy is approximately **504**, and rank-64 captures only **~28.5%** of total energy
- This indicates that task adaptation is NOT intrinsically low-rank in the traditional sense
- **Key Insight**: LoRA is effective not because it approximates the full spectrum, but because it **selectively learns the most critical singular directions for anomaly detection**
- Rank ablation shows <0.1%p performance variance across ranks 16-128, confirming that capturing the top singular directions is sufficient for effective adaptation
- The low-rank constraint acts as implicit regularization, preventing overfitting to training distributions
...
- This achieves complete isolation through physical preservation of $s_{base}$ with only **8% additional parameters per task**
...
1. **Zero Forgetting Guarantee**: First framework to achieve mathematically guaranteed zero forgetting (FM=0.0, BWT=0.0) in NF-based continual anomaly detection through complete parameter isolation
2. **Novel Reinterpretation of AFP**: We reinterpret the Arbitrary Function Property of Normalizing Flows as a structural foundation for parameter decomposition in continual learning, rather than its traditional interpretation as expressiveness freedom
3. **DeCoFlow Framework**: Coupling-level LoRA adaptation achieving practical parameter isolation without data storage. Only 8% parameter addition per task enables forgetting-free expansion
4. **Integral Components**: Distribution alignment (TSA), tail learning (TAL), and local expressiveness (ACL) improve the performance ceiling of low-rank adaptation by 3%p
5. **State-of-the-Art Performance**: 98.05% I-AUC on MVTec AD 15-class sequential learning (+3.35%p over previous SOTA), with 100% routing accuracy
...
We compare against comprehensive baselines including regularization-based (EWC), rehearsal-based (Replay), architecture isolation (PackNet), latest PEFT-based continual learning (L2P, DualPrompt), and AD-specific models (CADIC, ReplayCAD). Performance evaluation uses I-AUC/P-AUC for detection accuracy and **P-AP (Pixel Average Precision)** for localization precision, with **Forgetting Measure (FM)** and **Backward Transfer (BWT)** as core metrics for continual learning stability verification along with routing accuracy.
...
- P-AP gap (41.5% vs 26.6%) is discussed in Section 4.2.3 (Trade-off Analysis)
- Routing accuracy (95.8%) is lower than MVTec due to PCB1-PCB4 confusion (cosine similarity 0.78)

#### P-AP Performance Gap Analysis: Explicit Trade-off
...
**Key Finding**: Performance variance <0.1%p across ranks 16-128, confirming that LoRA effectively captures the critical singular directions regardless of exact rank, supporting our revised C2 claim.
...
| **MoLE6-ACL2** | **8** | **98.05** | **97.81** | **Stable** |
| MoLE10-ACL2 | 12 | 98.27 | 97.73 | Stable |

**Key Finding**: ACL blocks significantly improve training stability, allowing deeper architectures without collapse. MoLE6-ACL2 provides optimal performance-stability trade-off.
...
**Experimental Design**:
To validate that AFP enables superior adaptation compared to feature-level approaches, we compare:
1. **Coupling-level (DeCoFlow)**: LoRA adapters within NF coupling subnets
2. **Feature-level (L2P/DualPrompt style)**: Learnable prompts added to NF input features

**Hypothesis**: Feature-level adapters disrupt the density manifold by perturbing NF's input distribution, while coupling-level adapters leverage AFP to preserve density estimation integrity.

**Expected Outcome**:
| Approach | I-AUC | P-AP | Likelihood Consistency | FM |
|----------|-------|------|------------------------|-----|
| Coupling-level | ~98% | ~56% | High | 0.0% |
| Feature-level | ~93% | ~48% | Low (manifold distortion) | ~1% |

**Rationale**: AFP guarantees that coupling subnet modifications preserve NF validity. Feature-level modifications lack this guarantee, causing:
1. Distribution shift in NF input space
2. Degraded likelihood estimation accuracy
3. Potential gradient instability

*(Note: Full experimental results to be added upon completion)*

#### 4.4.3. SVD Analysis of Weight Updates

**Analysis Method**: We perform SVD on optimal weight updates $\Delta W_t^* = W_t^* - W_{base}$ obtained from full fine-tuning (unfrozen baseline).
...
Unlike the common "intrinsic low-rank" assumption, our analysis reveals that task adaptation is NOT inherently low-rank in the traditional sense. However, **LoRA remains effective** because:

1. **Selective Learning**: LoRA captures the most critical singular directions for anomaly detection, even if not covering the full spectrum
2. **Implicit Regularization**: The low-rank constraint prevents overfitting to training distributions
3. **Task-Specific Focus**: The top singular directions encode task-discriminative information most relevant for detection

**Supporting Evidence**: Rank ablation (Table 8) shows <0.1%p variance across ranks 16-128, confirming that capturing top directions is sufficient.
...
DeCoFlow achieves 98.05% I-AUC on MVTec-AD with FM=0.0% and BWT=0.0%, demonstrating that zero forgetting and high performance are achievable without data replay. The explicit trade-off between guaranteed stability and fine-grained localization (P-AP 55.8% vs CADIC's 58.4%) represents a principled design choice prioritizing long-term reliability.

---

## 6. Future Works

1. **Multi-resolution adaptation**: Addressing the P-AP gap through hierarchical feature extraction
2. **Contrastive routing**: Improving routing accuracy for highly similar task distributions
3. **Dynamic rank allocation**: Adaptive LoRA rank based on task complexity
4. **Theoretical analysis**: Formal bounds on LoRA's ability to capture critical singular directions
