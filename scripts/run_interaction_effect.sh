#!/bin/bash
# =============================================================================
# Interaction Effect Experiment: Proving WA, TAL, DIA are Integral Components
# =============================================================================
#
# Goal: Show that WA, TAL, DIA are NOT "generic boosters" but rather
#       "integral components that compensate for Base Freeze"
#
# Hypothesis:
...
#
# Expected Results:
#   - Trainable: WA/TAL/DIA effect is minimal (Base can self-adapt)
#   - Frozen: WA/TAL/DIA effect is large (Base cannot adapt, needs compensation)
# =============================================================================

set -e

# Configuration
CLASSES="bottle cable capsule carpet grid"  # 5 classes for faster iteration
EPOCHS=60
LR=3e-4
BATCH_SIZE=16
NCL=6  # MoLE blocks
...
LOG_DIR="./logs/InteractionEffect"
...
echo "=============================================="
echo "Interaction Effect Experiment"
echo "=============================================="
...
CUDA_VISIBLE_DEVICES=0 python run_moleflow.py ${COMMON_ARGS} \
    ...
    --no_dia \
    --experiment_name "IE_Trainable_Baseline" \
...
CUDA_VISIBLE_DEVICES=1 python run_moleflow.py ${COMMON_ARGS} \
    ...
    --no_dia \
    --experiment_name "IE_Trainable_WA" \
...
CUDA_VISIBLE_DEVICES=4 python run_moleflow.py ${COMMON_ARGS} \
    ...
    --no_dia \
    ${TAL_ARGS} \
    --experiment_name "IE_Trainable_TAL" \
...
CUDA_VISIBLE_DEVICES=5 python run_moleflow.py ${COMMON_ARGS} \
    ...
    --experiment_name "IE_Trainable_DIA" \
...
CUDA_VISIBLE_DEVICES=0 python run_moleflow.py ${COMMON_ARGS} \
    --no_whitening_adapter \
    --no_dia \
    --experiment_name "IE_Frozen_Baseline" \
...
echo "  python scripts/analyze_interaction_effect.py"
